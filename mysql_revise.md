...
1. 核心模式切換
	- Esc：進入命令模式。
		- （儲存格框線為藍色）
		- 儲存格左側的線會變細，且儲存格內沒有游標。
	- Enter：進入編輯模式。
		- （進入編輯狀態，框線為綠色）
		- 儲存格左側會有一條粗線（顏色通常隨你的佈景主題，如藍色或紫色），且儲存格內可以看到閃爍的游標。
...
- 手動建立 .venv 環境
    ...
    1.  重新指向 Kernel
        ...
        1.  選擇 "Python Environments..."
        1.  找到剛才建立的那個 `.venv` 路徑

- 手動指定解釋器 (Interpreter)
    1.  按下鍵盤 `Ctrl + Shift + P`
    1.  輸入 `Python: Select Interpreter` 並點選它
    1.  在選單中找路徑包含 `.venv` 的那一項（應該會顯示 `Python 3.14.4 ('.venv': venv)`），點選它
...
- 自動生成 `requirements.txt`
    -  在終端機左側有出現 (.venv)輸入
        ...
        1.  pip freeze：列出目前環境中所有安裝的套件與精確版本號。
        1.  \>：將輸出的內容導向（寫入）到檔案。
        1.  requirements.txt：目標檔案名稱（如果檔案已存在，會直接覆蓋）。


- 定時自動執行Python腳本
    - Scheduled Workflow 設定
        1.  在專案下，新增Folder，名稱 `.github`
        1.  在 .github 下，新增Folder，名稱 `workflows`
        1.  在 workflows 下，新增file，副檔名 `.yml`
            - 課程上 `scheduled.yml` 是老師給的
            - 修改 .yml 內容
                1.  上方第5列 - cron: '`0 0` * * *' # 每天國際標準時間 00：00 (台灣時間 08:00)
                    > 改為 - cron: '`59 15` * * *'   
					每天國際標準時間 00：00 (台灣時間 23:59)
                1.  下方倒數第3列 run: python `your_script_name.py`
                    > 改為專案要 run 的檔案，例： `pm25.py`
                1.  下方倒數第1列 `API_KEY`: ${{ secrets.`MY_API_KEY` }}
					> 將 .env 裡的所有內容一筆一筆寫入  
					> `name`: ${{  secrets.`name` }}  
					>> `age`: ${{  secrets.`age` }}
...
- 在GitHub使用自動化腳本
	...
	1.  (新畫面)左側欄位，點 `Secrets and variables`
		- 下拉選單，點 `Actions`
	1.  (新畫面)右側欄位最下方 **Repository secrets**，點綠色框 `New repository secret`
	1.  (新畫面)：將 .env 裡的 `key` = `values` 一筆一筆輸入
		1.  Name 輸入 `key`
		1.  Secret 輸入 `values`
		1.  按 Add secret
	1.  上方欄位，點 `Actions`
	1.  (新畫面)左側欄位第3列，會看到與 `scheduled.yml` 裡一樣的 name: **Scheduled Python Script** 點擊
	1.  點擊後，右側會看見藍色框裡的 `Run workflow` 點擊
	1.  (跳出視窗)點 `Run workflow`
	1.  (新畫面)點 `run-python`
